...
//    }
//
/// /    @Test /    @DisplayName("DM 목록 조회 - Limit 적용 성공 (최신순)") /    void
/// findDirectMessages_withLimit_success() { /        User sender = createUser("sender@test.com"); /
///        User receiver = createUser("receiver@test.com"); / /        Instant base = Instant.now();
/// / /        for (int i = 1; i <= 5; i++) { /            DirectMessage dm =
/// DirectMessage.builder() /                .sender(sender) /                .receiver(receiver) /
///               .content("메시지 " + i) /                .build(); / /
/// ReflectionTestUtils.setField(dm, "createdAt", base.plusSeconds(i * 10L)); /
/// directMessageRepository.save(dm); /        } / /        List<DirectMessageDto> result = /
///     directMessageRepository.findDirectMessages(sender.getId(), null, null, 3, null); / /
/// assertThat(result).hasSize(3); /        assertThat(result.get(0).getContent()).isEqualTo("메시지
/// 5"); /        assertThat(result.get(1).getContent()).isEqualTo("메시지 4"); /
