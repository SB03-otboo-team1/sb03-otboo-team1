...
public class FeedQueryService {
    ...
    public CursorPageResponseDto<FeedResponse> listFeeds(
        ...
        if (precipitationTypeEqual != null && !precipitationTypeEqual.isBlank()) {
            joinWeather = true;
            where.and(weather.precipitationType.eq(
                PrecipitationType.valueOf(precipitationTypeEqual.toUpperCase())));
        }
        ...
        // ---- Weather ----
        List<UUID> weatherIds = rows.stream().map(Feed::getWeatherId).filter(Objects::nonNull)
            .distinct().toList();
        ...
        if (!feedIds.isEmpty()) {
            List<Tuple> ootdTuples = qf
                .select(feedClothes.feed.id, clothes.id, clothes.name, clothes.imageUrl,
                    clothes.type)
                .from(feedClothes)
        ...
        if (hasNext && !rows.isEmpty()) {
            ...
            nextCursor = (sb == SortBy.CREATED_AT) ? last.getCreatedAt().toString()
