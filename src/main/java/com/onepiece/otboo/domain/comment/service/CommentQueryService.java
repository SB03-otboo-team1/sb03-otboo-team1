...
public class CommentQueryService {
     ...
     * 피드별 댓글 목록 조회 (커서 페이징).
     * <p>
     * 정렬: createdAt DESC, id DESC (tie-breaker) 커서 포맷: Base64(URL-safe) 로 인코딩된 "epochMilli:id" -
     * epochMilli: 마지막 행의 createdAt.toEpochMilli() (null이면 0) - id: 마지막 행의 UUID
     *
    ...
    public CursorPageResponseDto<CommentDto> listByFeed(UUID feedId, String cursor, UUID idAfter,
        int limit) {
        ...
        if ((decoded.createdAtLt == null || decoded.idLt == null) && idAfter != null) {
            ...
            if (!Objects.equals(pivot.getFeed().getId(), feedId)) {
                throw new IllegalArgumentException(
                    "idAfter does not belong to the feed: " + feedId);
            }
            Instant pivotCreatedAt = Objects.requireNonNullElse(pivot.getCreatedAt(),
                Instant.EPOCH);
    ...
    private String encodeCursor(Instant createdAt, UUID id) {
        String raw =
            createdAt.getEpochSecond() + CURSOR_SEP + createdAt.getNano() + CURSOR_SEP + id;
